...
tokenizer = BartTokenizerFast.from_pretrained('facebook/bart-large', cache_dir=f'{abs_root}/hf_cache')
...
# Load the model
model = BartForConditionalGeneration.from_pretrained(f"facebook/bart-large", cache_dir=f'{abs_root}/hf_cache')
...
for epoch in tqdm(range(num_epochs)):
    ...
    if best_valid_loss >= validation_loss:
        ...
        if hasattr(model, 'module'):
            model.module.save_pretrained(f"{abs_root}/bart-large-best")
        else:
            model.save_pretrained(f"{abs_root}/bart-large-best")
...
model.save_pretrained(f"{abs_root}/bart-large-final")
