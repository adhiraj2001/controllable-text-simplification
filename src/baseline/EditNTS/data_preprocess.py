...
def process_raw_data(comp_txt, simp_txt):
    ...
    # filter out the rows where the source sentence and the target sentence are identical to encourge editing
    # comp_txt, simp_txt= unzip([(i[0], i[1]) for i in zip(comp_txt,simp_txt) if i[0] != i[1]])
    # comp_txt, simp_txt= map(list, zip(*[(i[0], i[1]) for i in zip(comp_txt,simp_txt) if i[0] != i[1]]))
    ...
    def add_pos(df):
        ...
        pos_vocab = data.POSvocab(vocab_path='./vocab_data/')
